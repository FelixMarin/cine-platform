...
class JackettClient:
    ...
    def __init__(
        ...
        self.timeout = timeout or int(
            os.environ.get("JACKETT_TIMEOUT", settings.JACKETT_TIMEOUT)
        )
        self._session: Optional[aiohttp.ClientSession] = None

        logger.info(
            f"[Jackett] Cliente inicializado con URL: {self.url}, timeout: {self.timeout}s"
        )
    ...
    async def _make_request(
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.request(method, url, **kwargs) as response:
                    if response.status == 401:
                        ...
                        raise JackettError(
                            f"Error HTTP {response.status} de Jackett", response.status
                        )
            ...
            raise JackettError(
                f"Tiempo de espera agotado al conectar con Jackett ({self.timeout}s)"
            )
    ...
    async def search_movies(
        ...
        try:
            ...
            logger.info(
                f"[Jackett] Encontrados {len(results)} resultados para '{query}'"
            )
    ...
    def _parse_search_result(
        ...
        try:
            ...
            torrent_url = item.get(
                "DownloadUri", item.get("DownloadURL", item.get("Link", None))
            )
            ...
            publish_date = item.get(
                "PublishDate", item.get("publishDate", item.get("Date", None))
            )

            # Extraer indexador
            indexer = item.get(
                "Tracker", item.get("indexer", item.get("TrackerName", "Unknown"))
            )
    ...
    def format_results_for_frontend(
        self, results: List[JackettSearchResult]
    ) -> List[Dict[str, Any]]:
        """
