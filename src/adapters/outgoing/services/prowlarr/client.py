...
class ProwlarrClient:
    ...
    def __init__(self, url: Optional[str] = None, api_key: Optional[str] = None):
        ...
        self._timeout = int(os.environ.get("PROWLARR_TIMEOUT", 30))
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if status_code == 401:
                ...
                raise ProwlarrError(
                    f"Error HTTP {status_code} de Prowlarr", status_code
                )
        except requests.exceptions.RequestException as e:
            raise ProwlarrError(f"Error de comunicación con Prowlarr: {str(e)}")

    def search_movies(
        self, query: str, categories: Optional[List[int]] = None, limit: int = 20
        ...
        try:
            ...
            logger.info(
                f"[Prowlarr] Encontrados {len(results)} resultados para '{query}'"
            )
    ...
    def _format_relative_date(self, date_str: Optional[str]) -> str:
        ...
        try:
            ...
            if diff.days > 365:
                return (
                    f"hace {diff.days // 365} año{'s' if diff.days // 365 > 1 else ''}"
                )
            elif diff.days > 30:
                return (
                    f"hace {diff.days // 30} mes{'es' if diff.days // 30 > 1 else ''}"
                )
            elif diff.days > 0:
    ...
    def format_results_for_frontend(
        ...
        for result in results:
            ...
            is_magnet = result.magnet_url is not None and result.magnet_url.startswith(
                "magnet:"
            )
