...
from src.adapters.outgoing.repositories.postgresql.models.comment import (
    MovieComment,
    MovieCommentLike,
    MovieCommentReport,
)
...
class CommentRepository(CommentRepositoryPort):
    ...
    def get_comments_by_movie(
        ...
        query = query.filter(MovieComment.parent_comment_id is None)

        # Filtrar ocultos si no es admin
        if not include_hidden:
            query = query.filter(not MovieComment.is_hidden)
