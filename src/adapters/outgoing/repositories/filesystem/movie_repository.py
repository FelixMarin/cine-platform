...
class FilesystemMovieRepository(IMovieRepository):
    ...
    def __init__(self, base_folder: str = None, ttl_seconds: int = 300):
        ...
        """
        self._base_folder = base_folder or os.environ.get(
        ...
        self._index_file = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), ".movie_index_cache.json"
        )
        ...
        logger.info(
            f"FilesystemMovieRepository inicializado con TTL={ttl_seconds}s, base_folder={self._base_folder}"
        )
    ...
    def _ensure_cache(self) -> bool:
        ...
        with self._lock:
            ...
            if (
                ...
                or current_time - self._cache_timestamp > self._ttl_seconds
            ):
                ...
                logger.info(
                    f"Caché refrescada: {len(movies)} películas escaneadas en {scan_duration:.2f}s"
                )
        ...
        try:
            if os.path.exists(self._index_file):
                with open(self._index_file, "r") as f:
                    ...
                    logger.info(
                        f"Índice persistente cargado: {len(self._movie_index)} películas"
                    )
    ...
    def _parse_filename(self, filename: str) -> tuple:
        ...
        Soporta el patrón principal: Título (AÑO) [Calidad][Idioma].ext
        Ejemplos:
    ...
    def get_by_id(self, movie_id: Union[int, str]) -> Optional[Dict]:
        ...
        if isinstance(movie_id, int):
            ...
            if 0 <= movie_id < len(movies):
                ...
                duration = time.time() - start_time
                logger.info(
                    f"get_by_id({movie_id}): película '{movie.get('title')}' encontrada en {duration:.4f}s (índice)"
                )
                return movie
            duration = time.time() - start_time
        ...
        if movie:
            logger.info(
                f"get_by_id('{movie_id}'): película '{movie.get('title')}' encontrada en {duration:.4f}s (hash index)"
            )
        else:
            logger.info(f"get_by_id('{movie_id}'): no encontrada en {duration:.4f}s")
    ...
    def get_by_filename(self, filename: str) -> Optional[Dict]:
        ...
        for movie in self._movie_index.values():
            if movie.get("filename") == filename:
                duration = time.time() - start_time
                logger.info(
                    f"get_by_filename('{filename}'): encontrada en {duration:.4f}s"
                )
                return movie
    ...
    def search(self, query: str) -> List[Dict]:
        ...
        results = [m for m in movies if query_lower in m.get("title", "").lower()]
    ...
    def get_by_year(self, year: int) -> List[Dict]:
        ...
        duration = time.time() - start_time
        logger.info(
            f"get_by_year({year}): {len(results)} resultados en {duration:.4f}s"
        )
    ...
    def get_random(self, limit: int = 10) -> List[Dict]:
        ...
        duration = time.time() - start_time
        logger.info(
            f"get_random({limit}): {len(results)} resultados en {duration:.4f}s"
        )
    ...
    def get_recent(self, limit: int = 20) -> List[Dict]:
        ...
        duration = time.time() - start_time
        logger.info(
            f"get_recent({limit}): {len(results)} resultados en {duration:.4f}s"
        )
