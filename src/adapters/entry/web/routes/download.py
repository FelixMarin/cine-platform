...
def init_download_routes(prowlarr_client=None, transmission_client=None, jackett_client=None, torrent_optimizer=None):
    """
    ...
    if torrent_optimizer:
        ...
        from src.adapters.outgoing.services.ffmpeg.torrent_optimizer import TorrentOptimizer
...
def search_movie():
    ...
    if not query:
        return jsonify({'success': False, 'error': 'Parámetro de búsqueda "q" es requerido'}), 400
...
async def _parallel_search(query: str, limit: int) -> tuple:
    ...
    if prowlarr_task:
        ...
        source_info['prowlarr'] = {'results': len(prowlarr_results), 'success': prowlarr_success}
    
    if jackett_task:
        jackett_results, jackett_success = await jackett_task
        all_results.extend(jackett_results)
        source_info['jackett'] = {'results': len(jackett_results), 'success': jackett_success}
    
    if not source_info:
        logger.warning("[Parallel Search] ⚠️ No hay indexadores disponibles")
        source_info['error'] = 'No hay indexadores configurados'
    
    logger.info(f"[Parallel Search] ✅ Búsqueda completada: {len(all_results)} resultados")
...
def download_torrent():
    ...
    if not (url.startswith('magnet:') or url.startswith('http')):
        return jsonify({'success': False, 'error': 'URL debe ser magnet: o http(s)://'}), 400
    ...
    try:
        ...
        return jsonify({'success': False, 'error': f'Error al iniciar descarga: {str(e)}'}), 500
...
def download_status():
    ...
    try:
        ...
        if status_filter == 'active':
            ...
            torrents_data = [t for t in torrents_data if t.get('status') in [0, 1, 2, 3, 5]]
        ...
        for t in torrents_data:
            logger.info(f"[API] Torrent {t.get('id')}: {t.get('name')[:30]}... - Progress: {t.get('progress')}%")
            ...
            if size_total > 0:
                ...
                if abs(calculated_progress - progress) > 1:
                    logger.info(f"[API] Progreso: {progress}% → {calculated_progress:.1f}% ({size_downloaded}/{size_total})")
...
def optimize_status():
    ...
    if not _torrent_optimizer:
        return jsonify({
            'success': False,
            'error': 'Optimizador no disponible'
        }), 500
...
def get_categories():
    ...
    try:
        ...
        return jsonify({'success': False, 'error': 'Sin permisos para leer el directorio'}), 500
    except Exception as e:
    ...
    try:
        ...
        return jsonify({
            ...
            'optimization': {
                ...
                'logs': progress.logs[-1000:] if progress.logs else ''  # Últimos 1000 chars
            }
        })
...
def gpu_status():
    """
    Verifica disponibilidad de GPU en el worker FFmpeg
    """
    if not _torrent_optimizer:
        return jsonify({'success': False, 'error': 'Optimizador no disponible'}), 500
