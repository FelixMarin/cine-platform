...
def create_blueprints(auth_service, media_service, optimizer_service):
    ...
    def login():
        if request.method == 'POST':
            ...
            success, info = auth_service.login(email, password)
            if success:
    ...
    def play(filename):
        if not is_logged_in(): return redirect(url_for('main.login'))
        # Limpieza simple para el título
    ...
    def download_file(filename):
        if not is_logged_in(): return redirect(url_for('main.login'))
        file_path = media_service.get_safe_path(filename)
        if not file_path: return "Error", 404
    ...
    def optimizer():
        if not is_logged_in(): return redirect(url_for('main.login'))
        if not is_admin(): return "Acceso denegado", 403
        return render_template("optimizer.html")

    @bp.route("/process-file", methods=["POST"])
    def process_file():
        if not is_logged_in() or not is_admin(): return jsonify({"error": "No autorizado"}), 403
    ...
    def process_folder_route():
        ...
        if not folder or not os.path.exists(folder): return jsonify({"error": "Ruta inválida"}), 400
    ...
    def admin_manage():
        if not is_logged_in() or not is_admin(): return "Acceso denegado", 403
